    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if (playerIn.getHeldItem(hand) == null) {
            if (!world.isRemote && tileLaser.getConnectedLaser() != null) {
                playerIn.sendMessage(new TextComponentString("Has Connected Laser" + " " + tileLaser.getConnectedLaser().getPos()));
            }
            if (!world.isRemote && tileLaser.getConnectedMachine() != null) {
                playerIn.sendMessage(new TextComponentString("Has Connected Machine"));
            }
            return true;
        }
        return false;
